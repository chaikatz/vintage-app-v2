...
export default function VintageApp() {
  ...
  async function loadPosts() {
    ...
    if (postsData && postsData.length > 0) {
      ...
      const postsWithProfiles = postsData.map(post => ({
        ...post,
        profiles: profilesData?.find(p => p.id === post.user_id) || { username: 'Anonymous' }
      }))
      setPosts(postsWithProfiles)
      checkTodayMemories(postsWithProfiles)
    } else {
      setPosts([])
    }
  }
  ...
  function checkTodayMemories(allPosts) {
    ...
    const memories = allPosts.filter(post => {
      ...
      return postDate.getMonth() === today.getMonth() && postDate.getDate() === today.getDate() && postDate.getFullYear() < today.getFullYear()
    })
  ...
  async function handleSignUp() {
    ...
    if (data.user) {
      await supabase.from('profiles').insert({ id: data.user.id, username: username || email.split('@')[0], bio: 'Collecting moments' })
  ...
  async function handleLogin() {
    ...
    if (data.user) {
      ...
      if (!profile) { await supabase.from('profiles').insert({ id: data.user.id, username: email.split('@')[0], bio: 'Collecting moments' }) }
    }
  ...
  async function handlePhotoSelect(e) {
    ...
    reader.onload = (event) => { setUploadedImage(event.target.result); setUploadStep('filter') }
    reader.readAsDataURL(file)
    try {
      const exifr = (await import('exifr')).default
      const exif = await exifr.parse(file)
      setPhotoDate(exif?.DateTimeOriginal ? new Date(exif.DateTimeOriginal) : new Date())
    } catch { setPhotoDate(new Date()) }
  }
  ...
  async function handlePost() {
    ...
    const { error: postError } = await supabase.from('posts').insert({
      user_id: user.id, image_url: publicUrl, caption: uploadCaption, filter: selectedFilter,
      photo_date: photoDate?.toISOString(), date_stamp: formatDateStamp(photoDate || new Date())
    })
    if (postError) { showNotification('Failed to post'); return }
    setUploadedImage(null); setUploadedFile(null); setUploadCaption(''); setUploadStep('select')
    setSelectedFilter('slimAarons'); setPhotoDate(null); setScreen('feed')
    loadPosts()
  ...
  function handleStoryExport(post) {
    showNotification('Creating Story...')
    generateStoryImage(post.image_url, post.filter, post.date_stamp, (blob) => {
      ...
      showNotification('Saved! Share to Instagram')
    })
  }

  function getYearsAgo(date) { return new Date().getFullYear() - new Date(date).getFullYear() }
        ...
        <>
            ...
            <div className="p-4 fade-in">
              {todayMemories.length > 0 && showMemoryPrompt && (
                <div className="memory-card rounded-xl p-5 mb-5">
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <div className="text-lg font-semibold text-vintage-gold">🕰️ On This Day</div>
                      <div className="text-sm text-vintage-muted">{getYearsAgo(todayMemories[currentMemoryIndex].photo_date)} years ago</div>
                    </div>
                    <button onClick={() => setShowMemoryPrompt(false)} className="text-xl opacity-50">×</button>
                  </div>
                  <FilteredPhoto src={todayMemories[currentMemoryIndex].image_url} filter={todayMemories[currentMemoryIndex].filter} dateStamp={todayMemories[currentMemoryIndex].date_stamp} className="rounded-lg overflow-hidden mb-3" />
                  <p className="text-sm mb-3"><span className="font-semibold">{todayMemories[currentMemoryIndex].profiles?.username}</span> • {todayMemories[currentMemoryIndex].caption}</p>
                  <button onClick={() => handleStoryExport(todayMemories[currentMemoryIndex])} className="w-full p-3 bg-white border border-vintage-charcoal rounded-lg text-sm flex items-center justify-center gap-2"><ShareIcon /> Export to Story</button>
                </div>
              )}
              {posts.map(post => (
                <div key={post.id} className="bg-white rounded-lg p-3 mb-5 shadow-sm">
                  ...
                  <div className="flex items-center gap-4 mb-2">
                    <button onClick={() => handleLike(post.id)} className="text-xl">♡</button>
              ...
              {posts.length === 0 && (
                <div className="text-center py-20 text-vintage-muted">
                  ...
                  <button onClick={() => setScreen('upload')} className="text-vintage-charcoal underline">Post your first memory</button>
                </div>
              )}
            </div>
          )}
          ...
          {screen === 'upload' && (
            <div className="p-5 fade-in">
              ...
              {uploadStep === 'select' && (
                <label className="block border-2 border-dashed border-gray-300 rounded-lg p-10 text-center cursor-pointer">
                ...
                <>
                  ...
                  <div className="flex gap-2 overflow-x-auto pb-3 mb-5">
                    {Object.entries(FILTERS).map(([key, filter]) => (
                      <button key={key} onClick={() => setSelectedFilter(key)} className={`flex-shrink-0 px-4 py-2 rounded text-xs ${selectedFilter === key ? 'bg-vintage-charcoal text-white' : 'bg-gray-200'}`}>{filter.name}</button>
                    ))}
                  </div>
          ...
          {screen === 'profile' && (
            <div className="p-5 fade-in">
              <div className="text-center mb-8">
                <div className="w-20 h-20 rounded-full bg-gray-300 mx-auto mb-4 flex items-center justify-center text-3xl">{user?.email?.charAt(0).toUpperCase()}</div>
                <p className="font-semibold">{user?.email?.split('@')[0]}</p>
                <p className="text-sm text-vintage-muted italic">Collecting moments</p>
              </div>
              <div className="flex justify-center gap-10 py-5 border-y border-gray-300 mb-5">
                <div className="text-center"><p className="text-xl font-semibold">{posts.filter(p => p.user_id === user?.id).length}</p><p className="text-xs text-vintage-muted uppercase">Posts</p></div>
                <div className="text-center"><p className="text-xl font-semibold">0</p><p className="text-xs text-vintage-muted uppercase">Followers</p></div>
                <div className="text-center"><p className="text-xl font-semibold">0</p><p className="text-xs text-vintage-muted uppercase">Following</p></div>
              </div>
              <div className="grid grid-cols-3 gap-1">
                {posts.filter(p => p.user_id === user?.id).map(post => (
                  <div key={post.id} className="aspect-square overflow-hidden">
                    <FilteredPhoto src={post.image_url} filter={post.filter} className="h-full object-cover" />
                  </div>
                ))}
              </div>
          ...
          <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[430px] bg-vintage-cream border-t border-gray-300 flex justify-around py-4 z-50">
            {[{id:'feed',Icon:HomeIcon},{id:'search',Icon:SearchIcon},{id:'upload',Icon:PlusIcon},{id:'messages',Icon:MessageIcon},{id:'profile',Icon:ProfileIcon}].map(({id,Icon}) => (
              <button key={id} onClick={() => { setScreen(id); if(id==='upload'){setUploadStep('select');setUploadedImage(null)} if(id==='feed'){loadPosts()} }} className={`p-1 ${screen===id?'opacity-100':'opacity-40'}`}><Icon active={screen===id} /></button>
            ))}
          </div>
        </>
      )}
...
ENDOFFILEcat > ~/Downloads/vintage-app/app/page.js << 'ENDOFFILE'
'use client'
